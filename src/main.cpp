...
int main(int argc, char **argv) {
    ...
    try {
        ...
        for (int i = 0; i < num_runs; ++i) {
            ...
            // IMPORTANT:
            // Update your astar_interface.h/cpp to accept num_threads:
            // bool run_astar(const GridMap&, AStarState&, const std::vector<float>&, int num_threads);
            bool found = run_astar(map, state, hvals, num_threads);

            double ms = t.elapsed_ms();

            // Keep the first run's solution/path as "best" (same behavior as your original code)
            if (i == 0) {
                found_any = found;
                if (found) {
                    best_state = state;
